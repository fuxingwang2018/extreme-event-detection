...
class ExtremeDetectionAlgorithm(object):
    ...
    def optimized_threshold(self, filter_method, pctl_threshold, perc_of_days):
        """
        ...
        Args:
            ...
            pctl_threshold:
                Pre-defined N-th percentile (e.g. N = 95) for the
                entire chosen (sub)domain and time period
            perc_of_days:
                Percentage (e.g. 10%) of the days with the largest
                cr_sum as the potential events of interest
                (extreme events) to downscale them

        Returns:
            days_of_extreme:
                Days with extremes (e.g. the largest sums/numbers of event)
        """

        assert pctl_threshold > 0.0, "ERROR: negative pctl_threshold"
        assert perc_of_days > 0.0,   "ERROR: negative perc_of_days"

        # Finds candidate extreme events at each grid point and day as values
        ...
        days_of_extreme = (statistics_daily >= np.percentile(
            statistics_daily, 100 - perc_of_days))
